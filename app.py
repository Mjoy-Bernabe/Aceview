...
app.config['MYSQL_DB'] = 'auth_db'  # Replace with your actual database name

config = pdfkit.configuration(wkhtmltopdf='/usr/bin/wkhtmltopdf')
pdfkit.from_string("Hello PDF", "out.pdf", configuration=config)

mysql = MySQL(app)

# Other configurations like email, limiter, etc.
...
def preapp():
    ...
    # Handle POST - insert new application
    if request.method == 'POST':
        position = request.form.get('position')
        yearexperience = request.form.get('yearexperience')

        # Insert or update logic (if you want to update existing, you can adjust here)
        cur.execute("INSERT INTO applicants (user_id, position, yearexperience) VALUES (%s, %s, %s)",
                    (user_id, position, yearexperience))
        mysql.connection.commit()
        return redirect(url_for('preapp'))  # Avoid re-post on refresh

    # Handle GET - check if user already submitted application
    cur.execute("SELECT position, yearexperience FROM applicants WHERE user_id = %s ORDER BY id DESC LIMIT 1", (user_id,))
    ...
    # Flag to determine if application form is needed
    app_needed = False

    if applicant:
        position = applicant[0]
        yearexperience = applicant[1]
    else:
        app_needed = True
        position = None
        yearexperience = None

    # Qualification and other session values if you use them for results
    name = session.get('name')
    eligible_applicant = session.get('result')
    reason = session.get('reason')
    confidence = session.get('confidence')
